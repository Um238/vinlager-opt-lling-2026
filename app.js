// ============================================
// VINLAGER OPTÆLLING 2026 - APP.JS v108
// ============================================
console.log('========================================');
console.log('=== APP.JS SCRIPT START ===');
console.log('Version: v108 - FIX: Øl & Vand data persistence + Status rapport + Label scrolling/print');
    ...
    if (authObj && authObj.isLoggedIn && authObj.isLoggedIn()) {
      ...
      if (typeof loadOlVand === 'function') {
        loadOlVand().then(() => {
          ...
          // KRITISK: Opdater også filter dropdowns
          ...
          // KRITISK: Ved fejl, brug eksisterende data eller backup
          if ((!allOlVand || allOlVand.length === 0)) {
            try {
              const backup = localStorage.getItem('olVandBackup');
              if (backup) {
                allOlVand = JSON.parse(backup);
                console.log(`✅ Brugte ${allOlVand.length} Øl & Vand produkter fra backup ved auto-update fejl`);
              }
            } catch (e) {
          ...
          if (typeof populateLabelFiltersOlVand === 'function') populateLabelFiltersOlVand();
        });
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(() => {
    ...
    if (typeof authObj !== 'undefined' && authObj && authObj.isLoggedIn && authObj.isLoggedIn()) {
      ...
      // Load data med det samme - BÅDE Vin OG Øl & Vand
      if (typeof loadWines === 'function') {
        loadWines().catch(err => {
      ...
      // KRITISK: Load også Øl & Vand automatisk ved start
      if (typeof loadOlVand === 'function') {
        loadOlVand().catch(err => {
          console.error('❌ KRITISK FEJL i loadOlVand():', err);
          // Prøv at restore fra backup hvis API fejler
          try {
            const backup = localStorage.getItem('olVandBackup');
            if (backup) {
              const restored = JSON.parse(backup);
              allOlVand = restored;
              console.log(`✅ Gendannet ${allOlVand.length} Øl & Vand produkter fra backup ved start`);
              if (typeof updateDashboardOlVand === 'function') updateDashboardOlVand();
              if (typeof renderOlVandLager === 'function') renderOlVandLager();
              if (typeof populateLabelFiltersOlVand === 'function') populateLabelFiltersOlVand();
            }
      ...
      // KRITISK: Prøv alligevel at restore data fra backup selv uden login
      try {
        const backup = localStorage.getItem('olVandBackup');
        if (backup) {
          const restored = JSON.parse(backup);
          allOlVand = restored;
          console.log(`✅ Gendannet ${allOlVand.length} Øl & Vand produkter fra backup (uden login)`);
          if (typeof updateDashboardOlVand === 'function') updateDashboardOlVand();
          if (typeof renderOlVandLager === 'function') renderOlVandLager();
          if (typeof populateLabelFiltersOlVand === 'function') populateLabelFiltersOlVand();
        }
...
async function doImport(category = 'vin') {
  ...
  try {
    ...
    try {
      console.log('🔄 Opdaterer lokationer efter import...');
      // Hent alle produkter for at finde unikke lokationer
      const allProducts = await apiCall('/api/wines');
      const uniqueLocations = [...new Set((allProducts || []).map(p => p.lokation).filter(Boolean))];
      
      // Hent eksisterende lokationer
      const existingLocations = await apiCall('/api/locations').catch(() => []);
      const existingNames = (existingLocations || []).map(l => l.name);
      
      // Opret nye lokationer der ikke findes
      for (const locName of uniqueLocations) {
        if (!existingNames.includes(locName)) {
          try {
            // Bestem kategori baseret på navn
            const nameLower = locName.toLowerCase();
            const category = (nameLower.includes('øl') || nameLower.includes('vand') || 
                             nameLower.includes('bar') || nameLower.includes('køleren')) 
                             ? 'Øl & Vand' : 'Vin';
            
            await apiCall('/api/locations', {
              method: 'POST',
              body: JSON.stringify({ name: locName, category: category })
            });
            console.log(`✅ Oprettet ny lokation: ${locName} (${category})`);
          } catch (e) {
            console.warn(`⚠️ Kunne ikke oprette lokation ${locName}:`, e);
          }
        }
      }
      console.log('✅ Lokationer synkroniseret');
    } catch (e) {
      console.warn('⚠️ Kunne ikke synkronisere lokationer:', e);
    }
    ...
    if (category === 'ol-vand') {
      ...
      if (typeof loadOlVand === 'function') {
        ...
        // KRITISK: Sikr at backup er gemt efter import
        if (allOlVand && allOlVand.length > 0) {
          await saveOlVandBackup(allOlVand);
          console.log(`💾 Backup gemt efter import loadOlVand: ${allOlVand.length} produkter`);
        }
    ...
    renderLager();
      ...
      // KRITISK: Opdater også label filtre for Vin
      if (typeof populateLabelFilters === 'function') {
        populateLabelFilters();
      }
    }
...
async function loadOlVand() {
  try {
    ...
    // KRITISK: Gem backup ALTID, også hvis array er tom (så vi ved at vi har tjekket)
    await saveOlVandBackup(allOlVand);
    console.log(`💾 Backup gemt: ${allOlVand.length} produkter`);
      ...
      try {
        ...
        if (backup) {
          ...
          if (restored && Array.isArray(restored) && restored.length > 0) {
            ...
            console.warn('⚠️ Backup er tom eller ugyldig');
            allOlVand = [];
          }
        } else {
          console.warn('⚠️ Ingen backup fundet');
          allOlVand = [];
        }
      } catch (e) {
        console.warn('⚠️ Kunne ikke restore Øl & Vand backup:', e);
    ...
    // KRITISK: Opdater UI selv ved fejl
    if (typeof updateDashboardOlVand === 'function') {
      updateDashboardOlVand();
    }
    if (typeof renderOlVandLager === 'function') {
...
function updateDashboardOlVand() {
  // KRITISK: Restore fra backup hvis allOlVand er tom
  if (!allOlVand || !Array.isArray(allOlVand) || allOlVand.length === 0) {
    try {
      const backup = localStorage.getItem('olVandBackup');
      if (backup) {
        allOlVand = JSON.parse(backup);
        console.log(`✅ Gendannet ${allOlVand.length} Øl & Vand produkter fra backup i updateDashboardOlVand`);
      } else {
        allOlVand = [];
      }
    } catch (e) {
...
function renderOlVandLager() {
  ...
  if (!allOlVand || !Array.isArray(allOlVand) || allOlVand.length === 0) {
    try {
      ...
      if (backup) {
        allOlVand = JSON.parse(backup);
        console.log(`✅ Gendannet ${allOlVand.length} Øl & Vand produkter fra backup i renderOlVandLager`);
      } else {
        console.warn('⚠️ allOlVand er tom og ingen backup fundet');
        allOlVand = [];
      }
    } catch (e) {
      console.warn('⚠️ Kunne ikke restore backup:', e);
...
async function saveOlVandBackup(products) {
  // KRITISK: Gem også tom arrays så vi ved at vi har tjekket
  if (!products || !Array.isArray(products)) {
    console.warn('⚠️ saveOlVandBackup: products er ikke et array, gemmer tom array');
    products = [];
  }
  ...
  try {
    ...
    console.log(`💾 Øl & Vand backup gemt: ${products.length} produkter (${new Date().toLocaleString('da-DK')})`);
  } catch (e) {
    console.warn('⚠️ Kunne ikke gemme Øl & Vand backup:', e);
    // Prøv at gemme en mindre del hvis der er pladsproblemer
    try {
      const limited = products.slice(0, 1000); // Gem kun første 1000 hvis der er pladsproblemer
      localStorage.setItem('olVandBackup', JSON.stringify(limited));
      console.warn(`⚠️ Gemte kun første ${limited.length} produkter pga. pladsproblemer`);
    } catch (e2) {
      console.error('❌ Kunne ikke gemme backup overhovedet:', e2);
    }
  }
}
  ...
  try {
    ...
    if (!allOlVand || !Array.isArray(allOlVand) || allOlVand.length === 0) {
      ...
      if (!allOlVand || !Array.isArray(allOlVand) || allOlVand.length === 0) {
        ...
        try {
          // KRITISK: Kald loadOlVand og vent på at den er færdig
          if (typeof loadOlVand === 'function') {
            await loadOlVand();
            console.log(`✅ Efter loadOlVand: ${allOlVand ? allOlVand.length : 0} produkter`);
          } else {
            // Fallback: Hent direkte fra API
            console.log('⚠️ loadOlVand ikke tilgængelig - henter direkte...');
            const allProducts = await apiCall('/api/wines');
            const olVand = (allProducts || []).filter(p => {
              const kategori = (p.kategori || p.type || '').toLowerCase();
              const navn = (p.navn || '').toLowerCase();
              ...
              return vinId.startsWith('ol-') || /^w\d{4,}$/.test(vinId) ||
                     kategori.includes('øl') || kategori.includes('vand') || kategori.includes('sodavand') ||
                     navn.includes('øl') || navn.includes('vand');
            });
            allOlVand = olVand.map(p => {
              p.antal = parseInt(p.antal) || 0;
              p.minAntal = parseInt(p.minAntal) || 24;
            ...
            // Gem backup
            await saveOlVandBackup(allOlVand);
            console.log(`✅ Hentet ${allOlVand.length} produkter direkte fra API`);
          }
        } catch (loadError) {
    ...
    // KRITISK: Valider at data faktisk er der
    if (!allOlVand || !Array.isArray(allOlVand) || allOlVand.length === 0) {
      const errorMsg = 'Ingen Øl & Vand produkter fundet. Tjek at data er importeret og at backup eksisterer.';
      alert(errorMsg);
      console.error('❌ allOlVand er stadig tom efter alle forsøg');
      console.error('  Backup eksisterer:', !!localStorage.getItem('olVandBackup'));
      return;
    }
